...
const FaqCard: FC<FaqCardProps> = ({ open, toggle, title, subtitle, desc }) => {
  return (
    <div className="flex items-start w-full">
      <div
        className={`shadow-md rounded border border-t-0 ${
          open ? "border-blue-500" : ""
        } mt-5`}
      >
        <div className="p-4 text-xl relative font-medium">
          <div
            className="w-full pr-8 text-2xl text-[800] text-black"
            onClick={toggle}
          >
            {title}
          </div>
          <div className="text-gray-500 flex items-start justify-start text-sm  mt-2">
            {subtitle}
          </div>
          <button
        ...
        <Collapse isOpened={open}>
          <div className="p-5 pt-0">{desc}</div>
        </Collapse>
      </div>
    </div>
  );
